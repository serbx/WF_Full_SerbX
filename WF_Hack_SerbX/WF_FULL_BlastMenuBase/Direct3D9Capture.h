...
protected:
	std::map<IDirect3DDevice9*, std::shared_ptr<Renderer>> renderers;
	FuncHook endscene_hook;
	FuncHook reset_hook;

	void DoPresent(IDirect3DDevice9 *device);
	void DoReset(IDirect3DDevice9 *device);
...
public:
    ...
    virtual void ShowNMS(Renderer* rr);
	virtual void ShowStateXX(Renderer* rrr);
